import { useRef, useCallback, useEffect, useImperativeHandle, forwardRef, memo } from 'react';
import { useKeyboard } from '../hooks';
import { SCROLL_WHEEL_SCALE, SCROLL_WHEEL_DIVISOR } from '../constants';

export interface VideoCanvasHandle {
    /**
     * Drop the cached canvas rect and render geometry. A CSS transform (zoom/pan)
     * moves the canvas without firing the ResizeObserver, so the parent has to say
     * so explicitly or touch coordinates would be mapped through a stale rect.
     */
...
export const VideoCanvas = memo(
    forwardRef<VideoCanvasHandle, VideoCanvasProps>(function VideoCanvas(
        {
            isConnected,
            canvasRef,
            getVideoSize,
        ...
        const handlePasteRequest = useCallback(async () => {
            if (!isConnected || !onPasteText) {
                console.log(
                    '[VideoCanvas] handlePasteRequest aborted - not connected or no handler'
                );
                return;
            }
        ...
        // Let the parent invalidate the caches when it transforms the canvas.
        // Deliberately lazy: getCachedRect re-reads on the next interaction, so a
        // zoom/pan costs no forced layout here.
        useImperativeHandle(
            ref,
            () => ({
                invalidateGeometry: () => {
