...
### Where the wins actually are (nothing here is measured — see Verification for the baseline step)

The video pipeline owns **latency and smoothness**, not CPU. Estimated removable webview work is
~100–200 µs/frame ≈ **1 % of one core** at 8 Mbps/60 fps. Worth doing because it rides along with the
framing fix, but it is not where the CPU is. The CPU is in:

| Win | Est. magnitude | Effort |
|---|---|---|
| Hidden surfaces still running the full pipeline | **5–15 % of a core, continuously, for zero pixels** | ~20 lines |
| Chatty logcat (500 msg/s × 2000-entry filter × ~12 000 DOM nodes) | **10–50×** — currently pegs a core and janks | medium |
| App list: ~300 `dumpsys package` process spawns | **5–10×** — seconds → sub-second | medium |
| Video: fixed 8 ms batch delay | avg **~4 ms/frame**, ~25 % of a 60 fps frame period | medium |
| Video: multi-AU merge / orphaned read / overflow strand | correctness — reads as "laggy", unquantifiable | medium |
| Device poll: 7 spawns / 5 s | ~3 % of a core → ~0.5 % | small |
| Activation: 267 KB eager parse+eval | **10–30 ms** | small |

Landing order below follows this ranking, not the track numbering.
...
## Landing order — by value/effort, not by track number

Each row is one commit, independently revertible.

| # | Phase | Why here | Risk |
|---|---|---|---|
| 1 | **2A hidden-surface gating** + `ScrcpyPanel.onDidChangeViewState` | biggest CPU win, smallest diff, zero deps | low |
| 2 | **2B logcat batch IPC** | unblocks #3's benefit; ~50× fewer messages | low |
| 3 | **3A log virtualization** + debounce + row memo | the pathological path; 10–50× | medium, webview-only |
| 4 | **2C AppManager** rewrite + wire up the dead caches | seconds → sub-second, very visible | medium |
| 5 | 2D DeviceInfoService tiering, 2E getPreferredDevice caching | always-on background cost | medium |
| 6 | 1A extract `VideoFrameForwarder` (same semantics) + overflow-resync fix | video track starts; pure refactor + one bug fix | low |
| 7 | 1B read-loop rewrite (watchdog + `reader.cancel`) | fixes the swallowed-packet bug | low, host-only |
| 8 | **1C framed ArrayBuffer transport** (host + webview ship together) | owns latency + the merge bug | **high** |
| 9 | 1D rAF present loop + exception-safe `close()` | fixes the `VideoFrame` leak | medium |
| 10 | 1E decoder config + hardware fallback | medium |
| 11 | 1F backpressure signal | needs #6 and #8 | low |
| 12 | 2F `AdbCommandRunner` + socket backend, service by service | migrate *after* #4/#5 shrink the surface | medium |
| 13 | 3B pan/zoom, 3C prop identity | medium |
| 14 | 3D code-splitting, Track 4 activation + packaging | riskiest webview change; packaging is release-time | medium |

**Ordering notes:**
- Step 1 gates the panel with a plain `_isPanelVisible` flag checked inside the existing `onVideoData`
  callback — deliberately *not* via the forwarder, so it can land immediately without waiting on #6.
  Step 6 must then absorb that flag into `VideoFrameForwarder`'s `isDeliverable()` rather than leaving
  two gates.
- Steps 2 and 3 are separable but pair up: #2 alone still floods a non-virtualized list, #3 alone still
  pays 500 render passes/s. Land both before declaring the logcat path fixed.
- Step 8 cannot be split across host and webview — `ExtensionMessage.video.data` changes type from
  `string` to `ArrayBuffer`, so both sides must ship together (they do, in one `.vsix`).
- Steps 1–5 are all low-to-medium risk and touch nothing in the mirror hot path, so the riskiest work
  (#8) lands against an already-improved, already-verified baseline.
...
**Take a baseline before step 1 — nothing in this plan is measured yet.** Record all four; each maps to
a different claim in the ranking table above:
- **Idle CPU** — mirror panel in a background tab + hidden logcat panel, 60 s. Read
  `Developer: Show Running Extensions` and Task Manager. (Target: steps 1, 5, 12.)
- **Mirror latency** — mirror a device stopwatch against a wall clock, photograph both. (Target: step 8.)
- **Frame cadence** — 60 s run, note the `Rendered N frames` rate (temporarily re-enable `addLog`,
  which `MirrorApp.tsx:69-71` currently stubs to a no-op). (Target: steps 8, 9.)
- **App list wall time** and **logcat responsiveness under a chatty stream**. (Target: steps 2–4.)

Per step, in landing order:
1. **2A gating** — put a mirror panel in a background editor tab and hide a logcat panel. Host CPU must
   drop to idle (Task Manager / `Developer: Show Running Extensions`). Confirm clean resume on reveal.
2. **2B logcat batch** — chatty logcat (`am start` a heavy app). Entries still arrive in order, no
   duplicates, no gaps at the 50-entry batch boundary.
3. **3A virtualization** — same chatty stream at 2000 retained lines: UI stays responsive, typing in
   the search box doesn't stutter, autoscroll still tracks, scrollback still reaches the oldest line.
4. **2C AppManager** — time the app list before and after. Watch Task Manager for `adb.exe` churn.
   Verify the debug-apps list and app icons/labels are unchanged.
5. **2D / 2E** — 60 s idle mirror session: confirm spawn count per 5 s drops. Battery/storage/network
   still update (on their new tiers). Volume/lock/camera/paste still hit the right device.
6. **1A forwarder** — start/stop from both the sidebar *and* `Scrcpy: Start Screen Mirror` (the panel
   path, `extension.ts:37-39`). Toggle Persistent Mirroring, hide/show the sidebar in each state;
   expect byte-identical behaviour. Force the overflow path (temporarily set `MAX_VIDEO_BUFFER_SIZE`
   to 64 KB) and confirm the picture **recovers** instead of staying corrupt.
7. **1B read loop** — screen off >15 s then wake: no lost/garbage frame, no timeout spam. Stop
   mid-stream: clean exit, no unhandled rejection in the Debug Console.
8. **1C transport** — config arrives and the canvas lights up within ~1 s of `connected`. Scroll a long
   list, switch apps, **rotate the device** (exercises `sizeChanged`, `ScrcpyService.ts:189-192`).
   **Test touch accuracy at the screen corners with the device skin both on and off.** Zero decode
   errors in the webview console. Stutter under motion should be visibly gone. Re-run the stopwatch
   latency check against the baseline.
9. **1D rAF** — 5 min with no *"A VideoFrame was garbage collected without being closed"* warning.
   Toggle the device skin repeatedly (canvas remount): no leak, no stall. Frame cadence ≈ `maxFps`,
   not above. **Re-test touch** — this is where `getVideoSize` can silently break.
10. **1E decoder** — read the logged codec + acceleration line. Force the fallback by reordering
    candidates; confirm software decode renders. Force the error path; confirm sticky fallback
    recovers in ~1 s.
11. **1F backpressure** — CPU-throttle in webview devtools: `video-backpressure` fires **once** on
    entry (not per frame), picture degrades to keyframes-only, recovers cleanly.
12. **2F AdbCommandRunner** — every ADB-dependent feature works with mirroring **off** (spawn
    fallback) and **on** (socket backend). Test the switch mid-session: start mirroring while the file
    manager is open, then stop it.
13. **3B / 3C** — profile a pan in webview devtools: one commit per pointer-move, no forced-layout
    warnings, `VideoCanvas` no longer re-rendering per frame.
14. **3D / Track 4** — open all four views **from a packaged VSIX** and confirm none white-screens
    (CSP + chunk URLs). Confirm the packaged `dist/extension.js` is minified and no `.map` ships.
