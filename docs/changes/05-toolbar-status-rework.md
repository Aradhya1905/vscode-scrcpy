...
Shipped on `feature/token-cleanup-interaction`. All eleven plan steps landed.
Six things differ from, or go beyond, the plan text:

- **The device dropdown became a controlled component.** The plan's step 5
  moves the mirror view onto `components/DeviceSelector.tsx`, which owns its own
  `isOpen`. That silently removed something the inline copy did: the old
  `Toolbar.tsx:183-187` cleared both panels when the dropdown opened. Dropdown
  and panels occupy the same strip of screen, so all three have to be mutually
  exclusive. `DeviceSelector` now takes optional `isOpen` / `onOpenChange`; the
  toolbar passes them, and the three other apps keep the uncontrolled
  behaviour they already had.
- **The nav group's "collapse into the More menu" is CSS, not JavaScript.**
  `MorePanel` always renders a `.more-nav-fallback` navigation section;
  `toolbar.css` hides that section above 280px and hides `.toolbar-nav` below
  it, so exactly one of the two is ever visible. Measuring the sidebar in JS
  would have meant a resize observer feeding state into a `memo()`-wrapped
  toolbar — the thing [03](03-pan-rerender-perf.md) exists to prevent.
- **The status chip's fields collapse in priority order, not all at once.**
  The plan has one 200px rule. One rule is not enough: at 240–280px the full
  meta line still crowds the model name out of the chip. fps drops at 280px,
  resolution at 240px, and the whole meta line at 200px. Each field carries its
  own class so the breakpoints can target it.
- **The `·` separators in the mock are not in the shipped markup.** With fields
  disappearing at three different widths, any separator — typed into the string
  or generated by `+ ::before` — eventually renders as a leading `· 82%`.
  Fields are separated by `gap` alone.
- **`--phone-height` is gone, replaced by `--phone-aspect` and
  `--phone-frame-width`.** The plan named `phoneFrame.css:22-23`, but the frame
  actually on screen is `SamsungS20Frame`, and `deviceFrames.css` derived *its*
  width from `--phone-height` (`calc(var(--phone-height) * 0.4555)`) with a
  fixed-height screen. Both frames plus the unused `.phone-frame` now use
  `min(var(--phone-width), calc(100vw - 40px))` with an `aspect-ratio`. `100vw`
  is the webview viewport, which is the sidebar, so the frame tracks the drag.
- **Four hardcoded colours in `deviceSelector.css` became tokens.** The chip is
  now the most prominent surface in the toolbar, and its trigger background was
  `rgba(22, 27, 34, 0.65)` — a dark rectangle under a light theme, with
  theme-coloured text on it. The trigger, its hover state, the refresh button
  and `.device-item.selected` now resolve through the token layer. The dead
  `.toolbar-device-dropdown` rule, which only ever styled the inline dropdown
  this change deletes, went with them.

Smaller notes: `useSettingsStorage` replaces the never-read `toolbarAtBottom`
boolean with `toolbarPosition`, migrating an existing `false` to `'top'` rather
than resetting it. `useVideoDecoder`'s `reset()` pushes a 0×0 size as well as
clearing the ref, which is what makes the chip drop the resolution on
disconnect without `MirrorApp` tracking it separately. `.quick-action-btn`
gained a `:disabled` style — the panel had no disabled state before, and the
fallback nav buttons need one when nothing is streaming.

Verified: the `style={{` grep on `Toolbar.tsx` returns nothing,
`npm run compile:webview` builds, and `npm run typecheck && npm run lint &&
npm run format:check` are green. Not verified — these need a running Extension
Development Host and a human: the live chip against a real device (model,
resolution, fps, battery, and the charging bolt), dragging the sidebar through
all three breakpoints, the Top/Bottom setting flipping the panel animations,
Escape and outside-click on all three overlays with focus returning to the
trigger, screen-reader announcement of each button, and the render-count
comparison against the [03](03-pan-rerender-perf.md) pan baseline.
